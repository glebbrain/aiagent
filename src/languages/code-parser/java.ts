...
export class JavaCodeParser extends BaseCodeParser {
  protected methodRegex = "/(?:^\s*(?:public|private|protected|static|final|abstract)\s+)?(?:void|int|String|boolean|double|float|char)\s+([A-Za-z0-9_]+)\s*\(([^)]*)\)\s*(?:(?:->)|(?:\{[\s\S]*?\})|(?=;))/gm";
  ...
  protected fileExtension: string[] = [".java"];
}
